...
from keras.layers import Dense, Dropout, Activation, Input, Dot, Concatenate
...
x_train_h_description = np.load("train_h_description_pad.npy")
x_train_t_description = np.load("train_t_description_pad.npy")
...
embedding_layer = Embedding(embedding_matrix.shape[0],
									...
									input_length=maxlen,
									trainable=False)
h_description_embedding = embedding_layer(h_description_input)
t_description_embedding = embedding_layer(t_description_input)
...
r_description_conv1 = Conv1D(filters,kernel_size,padding='valid',activation='relu',strides=1)(r_description_embedding)
r_description_maxpool1 = GlobalMaxPooling1D()(r_description_conv1)
...
h_r = Concatenate(axis = -1)([h_description_maxpool1, r_description_maxpool1])

h_r_dense = Dense(filters)(h_r)
...
cos_sim = Dot(normalize = True, axes = 1)([h_r_dense, t_description_maxpool1])
output = Activation('sigmoid')(cos_sim)



model = Model(inputs = [h_description_input, t_description_input, r_description_input], outputs = output)
print(model.summary)
model.compile(loss='binary_crossentropy',
              optimizer='adam',
              metrics=['accuracy'])

model.fit([x_train_h_description, x_train_t_description, x_train_r], y_train,
          batch_size=batch_size,
          epochs=epochs,
          validation_split=0.2)
model.save('cnn_h_r.h5')